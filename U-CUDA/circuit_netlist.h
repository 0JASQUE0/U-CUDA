...
enum class NetlistDialect {
    // B-источники (поведенческие выражения): NGSpice, LTspice. Точнее и короче.
    Behavioral,
    // POLY у источника E — синтаксис SPICE 2, который понимают почти все, включая
    // Multisim. Тот отбрасывает B-источники при импорте, и узлы умножителей
    // остаются без источника: резисторы от них повисают одним концом.
    Poly,
};
...
struct NetlistOptions {
    ...
    NetlistDialect dialect = NetlistDialect::Behavioral;
    OpAmpModel     opamp;
