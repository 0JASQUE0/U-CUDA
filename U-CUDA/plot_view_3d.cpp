...
void Plot3DView::rebuild_axis_cache() {
    ...
    // если bbox не менялся - не пересоздавать
    ...
    bool fit_request)
{
    ...
    // 3. Размеры и отступы (3D не нужен margin под подписи осей, оси будут позже -
    //    пока выделим небольшой паддинг по краям, чтобы окно ImGui не упиралось в край)
    ...
    // 4. FBO render с depth
    camera.aspect = (float)plot_w / (float)plot_h;
    renderer.begin_frame(plot_w, plot_h, 0.05f, 0.05f, 0.08f, 1.0f, /*with_depth=*/true);
    float mvp[16];
    ...
    // подписи осей (текст рядом с концами линий-осей)
    {
        if (show_axes) {
            float xmn, xmx, ymn, ymx, zmn, zmx;
            if (series_cache_.bbox(xmn, xmx, ymn, ymx, zmn, zmx)) {
                ...
                // отступ от конца оси (в пикселях экрана)
                ...
                auto draw_axis_label = [&](float ex, float ey, float ez,
                    const char* name, ImU32 color)
                    {
                        ...
                        if (zclip > 1.0f) return; // точка за дальней плоскостью
    ...
    // 7. Зона интерактива (одна большая, на весь плот)
    ...
    // 10. Интерактив: orbit правой, pan левой, zoom колесом, popup-меню на короткий правый клик
    static bool rzoom_pending[64] = { false }; // упрощённый локал-стейт, чтобы не плодить поля.
    // (для production стоит вынести pending в поле класса, как в 2D)
    // Используем поля через локальный static с индексом owner_id - этого хватит для одного приложения.
    ...
    // Короткий правый клик (без drag) - popup-меню
