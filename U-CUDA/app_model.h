...
public:
    ...
    // true = последним действием был откат или повтор, и команда "диапазон"
    // после него ещё не выполнялась. Ctrl+Shift+T в этом состоянии берёт
    // значения ИЗ ПОЛЕЙ, а не из вида: отмена вернула поля, но картинка
    // осталась зумленной, и рассылка "из вида" затёрла бы восстановленное тем,
    // что осталось на экране. Сбрасывается любой новой командой (push_undo).
    bool undo_just_happened = false;
