...
struct NetNode {
    std::string label;
    // Коэффициент симметрии a[0] этого узла. Пусто = общий из конфига. Схемы
    // вроде CD/SEMP читают его как параметр МЕТОДА, и у сети с разными узлами
    // он такой же кандидат на расстройку, как и параметры системы.
    std::string symmetry_s;
...
struct NetworkConfig {
    ...
    // Делить вклад связи на число входящих дуг узла. Без этого узел с большой
    // степенью получает тягу, пропорциональную степени: у звезды центр видит N-1
    // соседей и расходится раньше, чем лучи вообще успевают синхронизоваться.
    // С делением K — это сила связи «на узел», одинаковая для центра и луча.
    // Выключается, когда нужен классический лапласиан (сумма без нормировки).
    bool normalize_coupling = true;
