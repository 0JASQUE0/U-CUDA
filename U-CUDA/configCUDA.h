...
// 6 = то же, что 5, но делённое на iterOfSynchr — скорость ЗА ЦИКЛ (log10(ratio)
//     растёт как m*log10 ρ, сравнима с линеаризацией только после деления);
// 7 = log10 ρ за цикл по схеме Беннеттина: AMOUNTOFX клонов на eps от мастера,
//     Грам-Шмидт и ренормировка после каждого цикла. Даёт спектральный радиус
//     оператора цикла напрямую, без крапа и без насыщения. Только unidirectional.
...
// Потолок числа пиков, которые peakFinder оставляет на одну точку свипа. Раньше здесь стояло
// предупреждение про local memory: значение задавало размер per-thread массивов next[] / labels[]
// в dbscan_optimized. Те ядра удалены как мёртвые, рабочий путь (dbscanCUDA -> dbscan) локальных
// массивов не держит, так что occupancy это больше не задевает — влияние осталось только на размер
// device-буферов пиков и интервалов.
...
// avgPeakFinderCUDA — feature codes. Источник истины для switch'а внутри kernel'а И для headless
// callers (без UI), которые должны явно указать, какие фичи писать в outAvgPeaks / AvgTimeOfPeaks.
// Host-side mirror — enum class BasinFeature в analysis_session.h (значения совпадают,
...
// Внутренняя лог-арифметика — ВСЕГДА в double, независимо от numb. Дело не в точности, а в
// согласованности host/device: при numb = float перегрузка log10/pow на host-стороне (MSVC math.h)
// выбирается double'ная, а в device-коде float'овая, и одна и та же формула давала РАЗНЫЕ значения
// узла (замерено ~1 float-ulp расхождения). В double-режиме — а это текущий numb — здесь ровно те
// же операции, что стояли в getValueByIdxLog, и значения не меняются ни на бит. Порядок операций
// тот же: шаг делится ДО умножения на k.
...
// Continuation-свип: у него СВОЯ конвенция узлов, не совпадающая с классической (там lerp, здесь
// lo + (hi-lo)*t). Менять её нельзя — по ней посчитаны все continuation-диаграммы. Точка k цепочки:
// forward — lo + (hi-lo)*t, reverse (гистерезисный проход от hi к lo) — hi - (hi-lo)*t, где
// t = k/denom, denom = nPts > 1 ? nPts-1 : 1 (при nPts == 1 даёт lo/hi).
// Была тремя рукописными копиями: kernels/*_cont.template.cu (device), cont_sweep_value в
// parametric_engine.cpp (CPU-continuation + CSV) и cont-ветка sweep_value_at в gui.cpp (оси
// графиков). Копии совпадали в double-режиме и расходились во float: host считал в double,
// device — в numb. Линейная ветка идёт в numb (как в ядре), лог-ветка внутри в double — по той же
// причине, что ucuda_node_log10_d.
