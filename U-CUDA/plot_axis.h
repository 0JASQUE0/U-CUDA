...
// Координата центра линии толщиной 1 px. ImGui рисует линию симметрично
// вокруг координаты: на целом значении она ложится половинками на два
// соседних пикселя и выглядит как размытая двухпиксельная — а соседний тик,
// попавший на .5, рисуется чётко. Отсюда ощущение, что часть чёрточек уехала
// на пиксель, причём обычно в одну сторону.
inline float px_line(float v) { return std::floor(v) + 0.5f; }
