...
struct AppConfig {
    ...
    // Кегль подписей на графиках — множитель к UI-шрифту, [0.5, 3.0], дефолт
    // 1.0 (прежний вид). Слайдер в Settings; при изменении дёргается
    // set_plot_font_scale() из plot_axis.h, как tick_precision у осей.
    ...
    // видимые: отсутствие ключа (конфиг от старой версии) = список остаётся
    // дефолтным, и новая вкладка появляется видимой сама.
    //
    // Дефолт не пустой: Network на свежей установке скрыта и включается
    // галочкой в Settings -> Tabs. Ключ в существующем конфиге перекрывает
    // его целиком, поэтому у того, кто вкладку уже включил, ничего не
    // изменится. Тот же дефолт продублирован в AppModel::hidden_tabs — туда
    // конфиг не доезжает, когда файла нет вовсе.
