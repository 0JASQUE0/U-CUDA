...
void Plot2DView::render(PlotRenderer& renderer,
    ...
    bool fit_request)
{
    // 1. Перезаливка кеша если поколение данных изменилось
    if (data_generation != series_generation) {
        ...
        // НЕ сбрасываем view_valid здесь: смена состава серий (вкл/выкл переменной
        // через show_var) не должна слетать зум. Автофит — только по fit_request
        // или при самом первом показе (view_valid стартует false).
        // Видимость: если число серий изменилось — берём из панели (init),
        // иначе сохраняем текущую (легенда автономна между пересчётами).
        if (count_changed) {
    ...
    if (visible.size() != series_in.size()) // страховка размера
        visible.resize(series_in.size(), true);

    // итоговая видимость серии: глобальная (галочки НУ) И локальная (легенда)
